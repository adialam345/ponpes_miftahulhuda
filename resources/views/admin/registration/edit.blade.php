...
@section('title', 'Edit Halaman Pendaftaran - Admin')
...
function setupFormValidation() {
    ...
    if (form) {
        form.addEventListener('submit', function(e) {
            ...
            Swal.fire({
                title: 'Memperbarui...',
                text: 'Sedang memperbarui halaman pendaftaran',
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between">
            <div class="mb-4 md:mb-0">
                <h1 class="text-3xl font-bold text-slate-900 flex items-center">
                    ...
                    Edit Halaman Pendaftaran
                </h1>
                <p class="text-slate-600 mt-1">Perbarui informasi halaman pendaftaran yang sudah ada</p>
            </div>
            <div class="flex flex-col sm:flex-row gap-3">
                <a href="{{ route('registration.show', ['type' => $registrationPage->page_type]) }}"
                   target="_blank"
                   class="inline-flex items-center justify-center px-4 py-2 bg-cyan-50 border border-cyan-300 rounded-lg shadow-sm text-sm font-medium text-cyan-700 hover:bg-cyan-100 hover:shadow-md transition-all duration-200">
    ...
    <form action="{{ route('admin.registration.update', $registrationPage->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="form-section fade-in">
                    ...
                    <div class="p-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="page-type-card {{ $registrationPage->page_type == 'pondok' ? 'selected' : '' }}" data-value="pondok">
                            ...
                            <div class="page-type-card {{ $registrationPage->page_type == 'smp' ? 'selected' : '' }}" data-value="smp">
                                <div class="flex items-center">
                        ...
                        <input type="hidden" name="page_type" id="page_type_hidden" value="{{ $registrationPage->page_type }}" required>
                    ...
                    <div class="p-6 space-y-6">
                        <div>
                            ...
                            <input
                                ...
                                name="title"
                                id="title"
                                value="{{ old('title', $registrationPage->title) }}"
                                required
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input
                                    ...
                                    value="{{ old('registration_start', $registrationPage->registration_start ? $registrationPage->registration_start->format('Y-m-d') : '') }}"
                                    class="input-field block w-full px-3 py-2 border border-slate-300 rounded-lg shadow-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all duration-200"
                                >
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    id="registration_end"
                                    value="{{ old('registration_end', $registrationPage->registration_end ? $registrationPage->registration_end->format('Y-m-d') : '') }}"
                                    class="input-field block w-full px-3 py-2 border border-slate-300 rounded-lg shadow-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all duration-200"
                                >
                ...
                <div class="form-section fade-in" style="animation-delay: 0.2s;">
                    ...
                    <div class="p-6">
                        <div class="border border-slate-300 rounded-xl overflow-hidden shadow-sm">
                            ...
                            >{!! old('content', $registrationPage->content) !!}</div>

                            <textarea id="hidden-content" name="content" class="hidden" required>{{ old('content', $registrationPage->content) }}</textarea>
                        </div>
                        ...
                        <div class="mt-4 p-4 bg-emerald-50 border border-emerald-200 rounded-lg">
                            <div class="flex items-start">
                                ...
                                <div class="text-sm text-emerald-800">
                                    <p class="font-medium">Tips Mengedit Halaman Pendaftaran:</p>
                                    <ul class="mt-1 list-disc list-inside space-y-1">
                                        <li>Gunakan template untuk menambahkan bagian baru dengan cepat</li>
                                        <li>Pastikan informasi persyaratan dan prosedur selalu up-to-date</li>
                                        <li>Periksa kontak pendaftaran secara berkala</li>
            ...
            <div class="lg:col-span-1">
                    ...
                    <div class="p-6">
                        <div class="space-y-4">
                            <div class="metadata-item">
                                <span class="text-sm font-medium text-slate-700">Status Pendaftaran:</span>
                                <span class="status-badge {{ \App\Http\Controllers\RegistrationController::isRegistrationOpen($registrationPage) ? 'status-open' : 'status-closed' }}">
                                    <i data-lucide="{{ \App\Http\Controllers\RegistrationController::isRegistrationOpen($registrationPage) ? 'check-circle' : 'clock' }}" class="w-3 h-3 mr-1"></i>
                                    {{ \App\Http\Controllers\RegistrationController::isRegistrationOpen($registrationPage) ? 'Terbuka' : 'Tertutup' }}
                                </span>
                            </div>

                            <div class="metadata-item">
                                <span class="text-sm font-medium text-slate-700">Terakhir diperbarui:</span>
                                <span class="text-sm text-slate-500">{{ $registrationPage->updated_at->format('d M Y H:i') }}</span>
                            </div>

                            <div class="metadata-item">
                                <span class="text-sm font-medium text-slate-700">Dibuat:</span>
                                <span class="text-sm text-slate-500">{{ $registrationPage->created_at->format('d M Y H:i') }}</span>
                            </div>
                        </div>
                ...
                <div class="form-section fade-in" style="animation-delay: 0.4s;">
                    <div class="section-header">
                        <h3 class="text-lg font-semibold text-slate-900 flex items-center">
                            <i data-lucide="eye" class="w-5 h-5 mr-2 text-cyan-600"></i>
                            Pratinjau
                        </h3>
                    </div>
                    <div class="p-6">
                        <div class="preview-card">
                            <div class="flex items-start">
                                <i data-lucide="monitor" class="w-5 h-5 text-cyan-600 mr-2 mt-0.5"></i>
                                <div class="text-sm text-cyan-800">
                                    <p class="font-medium">Lihat Hasil</p>
                                    <p class="mt-1">Klik tombol di bawah untuk melihat halaman pendaftaran di website.</p>
                                </div>
                            </div>
                        </div>
                ...
                <div class="form-section fade-in" style="animation-delay: 0.5s;">
                    <div class="section-header">
                        <h3 class="text-lg font-semibold text-slate-900 flex items-center">
                            <i data-lucide="help-circle" class="w-5 h-5 mr-2 text-emerald-600"></i>
                            Bantuan
                        </h3>
                    </div>
                    <div class="p-6">
                            ...
                            <div class="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                                <div class="flex items-start">
                                    ...
                                    <div class="text-sm text-blue-800">
                                        <p class="font-medium">Tips Editing:</p>
                                        <ul class="mt-1 list-disc list-inside space-y-1">
                                            <li>Gunakan Ctrl+S untuk menyimpan cepat</li>
                                            <li>Template membantu konsistensi format</li>
                                            <li>Periksa tanggal pendaftaran secara berkala</li>
                                            <li>Pastikan kontak selalu aktif</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="p-4 bg-amber-50 border border-amber-200 rounded-lg">
                                <div class="flex items-start">
                                    ...
                                    <div class="text-sm text-amber-800">
                                        ...
                                        <p class="mt-1">Perubahan akan langsung terlihat di website setelah disimpan.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        <input type="hidden" name="requirements[]" value="{{ !empty($registrationPage->requirements) ? $registrationPage->requirements[0] : '' }}">
        <input type="hidden" name="procedures[]" value="{{ !empty($registrationPage->procedures) ? $registrationPage->procedures[0] : '' }}">
        <input type="hidden" name="documents[]" value="{{ !empty($registrationPage->documents) ? $registrationPage->documents[0] : '' }}">
        <input type="hidden" name="contacts[]" value="{{ !empty($registrationPage->contacts) ? $registrationPage->contacts[0] : '' }}">
    </form>
</div>
</div>
@endsection
